...
LITELLM_API_BASE = os.getenv("OPENAI_BASE_URL")
...
class APILiteLLM(LanguageModel):
    ...
    def batched_generate(self, convs_list: list[list[dict]],
        ...
        if LITELLM_API_BASE and not self.use_open_source_model:
            completion_kwargs["api_base"] = LITELLM_API_BASE

        outputs = litellm.batch_completion(**completion_kwargs)
...
class LocalHFAttackLLM(LanguageModel):
    """Loads a HuggingFace model locally on GPU for use as the PAIR attacker."""

    def __init__(self, model_name):
        super().__init__(model_name)
        hf_model_id = self.model_name.value
        self.tokenizer = AutoTokenizer.from_pretrained(hf_model_id)
        if self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
        self.tokenizer.padding_side = "left"
        # Fall back to CPU if there is less than 4 GB of free GPU memory so
        # that a large target model loaded on the same GPU does not cause OOM.
        if torch.cuda.is_available():
            free_gpu_gb = torch.cuda.mem_get_info()[0] / (1024 ** 3)
            device_map = "auto" if free_gpu_gb >= 4 else "cpu"
        else:
            device_map = "cpu"
