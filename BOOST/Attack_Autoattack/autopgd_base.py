...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None,attention_mask=None, token_type_ids=None):
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    logits = self.model(inputs_embeds=x_adv).logits
        ...
        for i in range(self.n_iter):
            ...
            for _ in range(self.eot_iter):
                if not self.is_tf_model:
                    with torch.enable_grad():
                        logits = self.model(inputs_embeds=x_adv,attention_mask=attention_mask, token_type_ids=token_type_ids).logits
                        loss_indiv = criterion_indiv(logits, y)
                        loss = loss_indiv.sum()
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None, attention_mask=None, token_type_ids=None):
        """
        ...
        if not self.is_tf_model:
            tmp = self.model(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids)
            if hasattr(tmp, "logits"):
                logits = tmp.logits
            else:
                logits = tmp  # fallback if already tensor
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, y=None, x_init=None, attention_mask=None, token_type_ids=None):
        """
        :param x:           clean images
        :param y:           clean labels, if None we use the predicted labels
        """
        ...
        x = x.detach().clone().float().to(self.device)
        if not self.is_tf_model:
            y_pred = self.model(inputs_embeds=x, attention_mask=attention_mask, token_type_ids=token_type_ids).logits.max(1)[1]
        else:
            y_pred = self.model.predict(x).max(1)[1]
        if y is None:
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        output = self.model(inputs_embeds=x_to_fool,attention_mask=attention_mask, token_type_ids=token_type_ids).logits
                    else:
