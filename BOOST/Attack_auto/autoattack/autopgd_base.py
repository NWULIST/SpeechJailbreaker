...
def L1_projection(x2, y2, eps1, lower_bound=0., upper_bound=1.):
    '''
    ...
    and lower_bound <= x2 + y2 + delta <= upper_bound
    '''

    x = x2.clone().float().view(x2.shape[0], -1)
    y = y2.clone().float().view(y2.shape[0], -1)
    sigma = y.clone().sign()
    u = torch.min(upper_bound - x - y, x + y - lower_bound)
...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'Linf':
            ...
            t = torch.randn(x.shape).to(self.device).detach()
            delta = L1_projection(x, t, self.eps, lower_bound=self.bounds[0], upper_bound=self.bounds[1])
            x_adv = x + t + delta
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1,
                        x - self.eps), x + self.eps), self.bounds[0], self.bounds[1])
                    ...
                    x_adv_1 = torch.clamp(x + self.normalize(x_adv_1 - x
                        ...
                        L2_norm(x_adv_1 - x, keepdim=True)), self.bounds[0], self.bounds[1])
                    ...
                    delta_u = x_adv_1 - x
                    delta_p = L1_projection(x, delta_u, self.eps, lower_bound=self.bounds[0], upper_bound=self.bounds[1])
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        assert self.loss in ['ce', 'dlr', 'ce-targeted'] #'ce-targeted-cfts'
    ...
    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):
        ...
        if not use_rs:
            ...
            x_init += L1_projection(x, x_init - x, 1. * float(epss[0]), lower_bound=self.bounds[0], upper_bound=self.bounds[1])
        ...
        for eps, niter in zip(epss, iters):
            ...
            #
            if not x_init is None:
                x_init += L1_projection(x, x_init - x, 1. * eps, lower_bound=self.bounds[0], upper_bound=self.bounds[1])
...
class APGDAttack_targeted(APGDAttack):
    def __init__(
        ...
        super(APGDAttack_targeted, self).__init__(predict, n_iter=n_iter, norm=norm,
            ...
            use_largereps=use_largereps, is_tf_model=is_tf_model, logger=logger, bounds=bounds)
