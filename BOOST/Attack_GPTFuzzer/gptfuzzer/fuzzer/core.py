...
class PromptNode:
    ...
    def num_jailbreak(self):
        if isinstance(self.results, list) and len(self.results) >= 1:
            return sum(1 for item in self.results if item.success is not False)
        else:
            return 0 if self.results.success is False else 1

    @property
    def num_reject(self):
        print(self.results)
        if isinstance(self.results, list) and len(self.results) >= 1:
            num = sum(1 for item in self.results if item.success is not False)
            lens = len(self.results)
        else:
            num = 0 if self.results.success is False else 1
            lens = 1
        return lens - num
        ...
        if messages:
            ...
            for i, response in zip(valid_prompt_indices, responses):
                ...
                if self.evaluation != 'strongreject':
                    ...
                    prompt_nodes[i].results = self.predictor([response], self.questions[0], None)
