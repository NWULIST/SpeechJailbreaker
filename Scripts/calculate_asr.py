...
def calculate_asr(csv_path):
    ...
    # ---------------------------------------------------------
    # Level 3: Success Distribution by Iteration Level
    # ---------------------------------------------------------
    # Filter the dataframe to only include successful attempts
    successful_df = df[df['is_success']]
    

    # Find the exact iteration where each index FIRST succeeded
    first_success_per_index = successful_df.groupby('index')['iteration'].min()
    
    # Count how many unique indices succeeded at each iteration depth
    success_counts_by_iteration = first_success_per_index.value_counts().sort_index()
    ...
    print(f"--- Level 3: Successes by Iteration Level ---")
    for iteration, count in success_counts_by_iteration.items():
        pct_of_successes = (count / successful_indices_count) * 100
        print(f"Depth/Iteration {iteration}: {count} successes ({pct_of_successes:.1f}% of all successful attacks)")
