...
if [ "$ADD_EOS" = "True" ]; then
    LOG_PATH="Logs/${MODEL_PATH}/SURE_eos-${RUN_INDEX}"
else
    LOG_PATH="Logs/${MODEL_PATH}/SURE-${RUN_INDEX}"
fi
...
(
      ...
      echo "CMD: CUDA_VISIBLE_DEVICES=$FREE_GPU python -u $PYTHON_SCRIPT --defence $defence  --target_model $MODEL_PATH $ADD_EOS_FLAG --guard $guard --evaluation $EVALUATION${EOS_NUM:+ --eos_num $EOS_NUM} --harmful_dataset $HARMFUL_DATASET --targets_dataset $TARGETS_DATASET  --num_tasks  $NUM_TASKS > ${LOG_PATH}/0.log 2>&1" >> ${LOG_PATH}/0.log
      CUDA_VISIBLE_DEVICES=$FREE_GPU python -u "$PYTHON_SCRIPT" --defence $defence --target_model $MODEL_PATH $ADD_EOS_FLAG  --guard $guard --evaluation $EVALUATION${EOS_NUM:+ --eos_num $EOS_NUM} --harmful_dataset "$HARMFUL_DATASET" --targets_dataset "$TARGETS_DATASET"  --num_tasks  $NUM_TASKS > "${LOG_PATH}/0.log" 2>&1
